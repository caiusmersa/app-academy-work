...
describe TicTacToeNode do
  ...
  describe '#possible_moves' do
    it "generates 9 possible_moves for an empty board" do
      expect(empty_board_node.possible_moves.length).to eq(9)
    end

    it "all their marks' are the opposite of their parents'" do
      expect(empty_board_node.possible_moves.all? do |kid|
        kid.next_mover_mark == :o
      end).to eq(true)
    end

    it "all their #prev_mov_pos values are their parent's" do
      child_prev_moves = empty_board_node.possible_moves.map{ |kid| kid.prev_move_pos }
      positions = [0,1,2].product([0,1,2])
      expect(child_prev_moves).to match_array(positions)
    end

    it "the possible_moves's boards are dups of the parent's" do
      kid_boards = empty_board_node.possible_moves.map{ |kid| kid.board }
      expect(
    ...
    it "doesn't produce possible_moves that include non-empty squares" do
      empty_board_node.board[[0,0]] = :x
      empty_board_node.board[[0,1]] = :o
      kids = empty_board_node.possible_moves.map{ |kid| kid.prev_move_pos }
