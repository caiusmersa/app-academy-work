...
def top_track_counts
  ...
  ORDER BY COUNT(tracks.album) DESC
  LIMIT 10
  SQL
end
...
def expensive_tastes
  ...
  execute(<<-SQL)
      ...
      SELECT styles.style, AVG(albums.price/a_tracks.track_count),
      albums.price/a_tracks.track_count
      FROM styles
      JOIN
      (SELECT
        albums.asin as asin, COUNT(tracks.song) as track_count
      FROM
        tracks
      JOIN
        albums
      ...
      GROUP BY
        albums.asin) as a_tracks
      ON
        styles.album = a_tracks.asin
      GROUP BY styles.style
      ORDER BY AVG(albums.price/a_tracks.track_count) DESC
      LIMIT 5
